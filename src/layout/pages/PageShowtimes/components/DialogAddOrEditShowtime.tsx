...
const DialogAddOrEditShowtime: FunctionComponent<IDialogAddOrEditShowtimeProps> = (props) => {
  ...
  const [errors, setErrors] = useState<ShowtimeValidation>({ movieId: '', price: '', roomId: '', screenTypeId: '' });
  ...
  const onDialogEnter = () => {
    ...
    setErrors({ movieId: '', price: '', roomId: '', screenTypeId: '' });
	  setRequestError('');
  }
  ...
  const validateInput = () : boolean => {
    let validationResult: ShowtimeValidation = { movieId: '', price: '', roomId: '', screenTypeId: '' };
    let isOK = true;
    ...
    if (!(showtimeInput.price > 0)) {
      validationResult.price = Constants.ERROR_MSG_FIELD_NOT_POSITIVE_NUMBER;
      isOK = false;
    }
